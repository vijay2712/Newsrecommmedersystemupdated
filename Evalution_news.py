...
class precision_recall_calculator():
    ...
    def __init__(self, test_data, train_data, pm, is_based):
        self.test_data = test_data
        self.train_data = train_data
        self.user_test_sample = None
        self.model1 = pm
        self.model2 = is_based
    ...
    def get_test_sample_recommendations(self):
        ...
        for user_id in self.users_test_sample:
            ...
            self.ism_training_dict[user_id] = list(user_sim_items["Article_ID"])
    
            #Get items for user_id from popularity model
            user_sim_items = self.model1.recommend(user_id)
            self.pm_training_dict[user_id] = list(user_sim_items["Article_ID"])
    
            #Get items for user_id from test_data
            test_data_user = self.test_data[self.test_data['Article_ID'] == user_id]
            self.test_dict[user_id] = set(test_data_user['Article_ID'].unique() )
